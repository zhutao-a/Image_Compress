...
static char write_position;
static uint32_t byte_count;
static uint8_t current_component;
static int num = 0;

static void Initializations(const uint8_t* data, uint16_t w, uint16_t h, int coding_mode)
{
	uint32_t i = 0;
	uint32_t j = 0;
	uint8_t k = 0;
...
static void RunModeProcessing()
{
	...
	printf("RUNcnt=%d  ", enc.RUNcnt);
	printf("enc.Ix=%d  enc.Ra=%d  enc.Rb=%d  enc.Rc=%d  enc.Rd=%d  ", enc.Ix, enc.Ra, enc.Rb, enc.Rc, enc.Rd);
 	...
 	if(enc.Ra != enc.Ix)
	{
		...
		printf("enc.A[enc.q]=%d  enc.N[enc.q]=%d  enc.Nn[enc.q]=%d\r\n", enc.A[enc.q], enc.N[enc.q], enc.Nn[enc.q]);
		...
		printf("enc.RItype=%d  ", enc.RItype);
		printf("enc.SIGN=%d  ", enc.SIGN);
		printf("enc.TEMP=%d  ", enc.TEMP);
		printf("enc.map=%d  ", enc.map);
		printf("EMErrval=%d,enc.k=%d", enc.EMErrval, enc.k);
...
static void Prediction()
{
	...
	printf("enc.Q[0]=%d  ", enc.Q[0]);
	printf("enc.Q[0]=%d  ", enc.Q[1]);
	printf("enc.Q[0]=%d  ", enc.Q[2]);
	printf("enc.SIGN=%d  ", enc.SIGN);
	printf("enc.q=%d  ", enc.q);
	printf("enc.A[enc.q]=%d  enc.B[enc.q]=%d  enc.C[enc.q]=%d  enc.N[enc.q]=%d\r\n", enc.A[enc.q], enc.B[enc.q], enc.C[enc.q], enc.N[enc.q]);
	printf("enc.Px=%d  ", enc.Px);
	printf("enc.Errval=%d  ", enc.Errval);
}
