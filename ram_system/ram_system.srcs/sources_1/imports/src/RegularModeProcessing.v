...
a_ram13x365 a_ram13x365_u0 (
	//写通道
	.clka 	(clk			),// input wire clka
	.ena  	(1'b1			),// input wire ena
	.wea  	(stream_valid	),// input wire [0 : 0] wea
	.addra	(q_ff1			),// input wire [8 : 0] addra
	.dina 	(Aq_updated		),// input wire [12 : 0] dina
	//读通道
	.clkb 	(clk			),// input wire clkb
	.enb  	(regular_valid	),// input wire enb
	.addrb	(q				),// input wire [8 : 0] addrb
	.doutb	(Aq_read		) // output wire [12 : 0] doutb
);

b_ram9x365 b_ram9x365_u0 (
	//写通道
 	.clka  	(clk			),// input wire clka
 	.ena   	(1'b1			),// input wire ena
 	.wea   	(stream_valid	),// input wire [0 : 0] wea
 	.addra 	(q_ff1			),// input wire [8 : 0] addra
 	.dina  	(Bq_updated		),// input wire [8 : 0] dina
	//读通道
 	.clkb 	(clk			),// input wire clkb
 	.enb  	(regular_valid	),// input wire enb
 	.addrb	(q				),// input wire [8 : 0] addrb
 	.doutb	(Bq_read		) // output wire [8 : 0] doutb
);

c_ram8x365 c_ram8x365_u0 (
	//写通道
	.clka  	(clk			),// input wire clka
	.ena   	(1'b1			),// input wire ena
	.wea   	(stream_valid	),// input wire [0 : 0] wea
	.addra 	(q_ff1			),// input wire [8 : 0] addra
	.dina  	(Cq_updated		),// input wire [7 : 0] dina
	//读通道
	.clkb 	(clk			),// input wire clkb
	.enb  	(regular_valid	),// input wire enb
	.addrb	(q				),// input wire [8 : 0] addrb
	.doutb	(Cq_read		) // output wire [7 : 0] doutb
);

n_ram7x365 n_ram7x365_u0 (
	//写通道	
 	.clka  	(clk			),// input wire clka
 	.ena   	(1'b1			),// input wire ena
 	.wea   	(stream_valid	),// input wire [0 : 0] wea
 	.addra	(q_ff1			),// input wire [8 : 0] addra
 	.dina  	(Nq_updated		),// input wire [6 : 0] dina
	//读通道
 	.clkb 	(clk			),// input wire clkb
 	.enb  	(regular_valid	),// input wire enb
 	.addrb	(q				),// input wire [8 : 0] addrb
 	.doutb	(Nq_read		) // output wire [6 : 0] doutb
);
